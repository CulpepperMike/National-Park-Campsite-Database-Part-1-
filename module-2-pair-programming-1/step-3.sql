...
-- (expected: 3 rows, startng with "Cuyahoga Valley")
...
-- select all columns from site where utilities is true and order by max RV length with the largest
-- coming first
-- (expected: 26 rows, starting with 35-foot max_rv_length)
select site_id, campground_id, site_number, max_occupancy, accessible, max_rv_length, utilities from site
where utilities = 'true' order by max_rv_length desc;
...
where from_date between '11/01/2023' and '11/30/2023';
...
-- select reservation id, site id, name of the reservations where site id is in the list 9, 20, 24,
-- 45, 46 (expected: 16 rows)
